...
(defun run-parse ()
  ...
  (let* ((results '())
    ...
    (while (not (or (parsec-error-p result) (equal parsed-kind :eof)))
      ...
      (pcase parsed-kind
        (:close-bracket (let* ((ns (car namespaces))
                               (ns-brackets (cdr ns))
                               (open-brackets (cadr next-state)))
                          (when (< open-brackets ns-brackets)
                            (setq namespaces (rest namespaces))))
                        )
        (:describe (let* ((ns (car namespaces))
                          (ns-title (car ns))
                          (ns-brackets (cdr ns))
                          (open-brackets (cadr next-state))
                          (new-ns (string-join (list ns-title parsed-value) (unless (string-empty-p ns-title) "/"))))
                     (setq namespaces (append `((,new-ns . ,open-brackets)) namespaces))
                     (setq results (append `(,new-ns) results))
                     ))
        (:test (let* ((ns (car namespaces))
                      (ns-title (car ns))
                      (new-ns (string-join (list ns-title parsed-value) (unless (string-empty-p ns-title) "/"))))
                 (setq results (append `(,new-ns) results))
                 ))
